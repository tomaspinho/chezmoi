...
Item {
    ...
    // signalStrength is a 0-1 fraction, so this maps it across all five
    // glyphs. Rounding rather than flooring-plus-one: the latter could never
    // return index 0, leaving the no-signal glyph unreachable and lumping
    // everything under 25% in with the one-bar case.
    function barsFor(strength) {
        return bars[Math.max(0, Math.min(4, Math.round(strength * 4)))];
    }
    ...
    BarIcon {
        ...
        // as noticeably smaller at the same pixelSize. BarIcon keeps that
        // from inflating the bar row's height - see the note there.
        oversize: 4
        ...
        color: wifi.active ? Theme.colCyan
            : wifi.wiredConnected ? Theme.colCyan
            : Networking.wifiEnabled ? Theme.colYellow
            : Theme.colMuted
        glyph: wifi.active ? wifi.barsFor(wifi.active.signalStrength)
    ...
    BarPopup {
        ...
        function close() {
            wifi.expanded = false;
            pending = null;
            // Cleared here too, not just in activate(): a failure message left
            // behind would otherwise still be sitting there the next time the
            // popup is opened, long after it stopped being true.
            error = "";
        }
        ...
        PopupHeader {
            ...
            onLabel: "󰖩 on"
            offLabel: "󰖪 off"
            // A hardware rfkill switch leaves the state readable but not
            // settable from here.
        ...
        ListView {
            ...
            delegate: Rectangle {
                ...
                Text {
                    ...
                    color: modelData.connected ? Theme.colCyan : Theme.colMuted
                ...
                Text {
                    ...
                    color: modelData.connected ? Theme.colCyan
                        : modelData.known ? Theme.colBlue : Theme.colFg
                    font { family: Theme.fontFamily; pixelSize: Theme.fontSize - 2 }
                }
        ...
        Item {
            ...
            Rectangle {
                anchors.fill: parent
                anchors.topMargin: 4
                color: Qt.lighter(Theme.colBg, 1.6)
                radius: 4
        ...
        Item {
            ...
            Text {
                ...
                color: wifi.wiredConnected ? Theme.colCyan : Theme.colFg
                font { family: Theme.fontFamily; pixelSize: Theme.fontSize - 2 }
            }
            ...
            Text {
                ...
                color: wifi.wiredConnected ? Theme.colCyan : Theme.colMuted
                font { family: Theme.fontFamily; pixelSize: Theme.fontSize - 2 }
